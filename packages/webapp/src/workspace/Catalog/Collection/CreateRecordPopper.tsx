...
import { CollectionRecord, CatalogRecord } from "@colab/store/lib/records";
import { useInput } from "src/utils";
import { useCollectionActions } from "@workspace/Catalog/hooks";

interface ICreateRecordPopper {
    collection: CollectionRecord;
    onClose: (event: any, reason: string) => void;
}

interface IDropdownPopper {
    templates: CatalogRecord["templates"];
...
export default Popper.create<HTMLDivElement, ICreateRecordPopper>(
    ({ collection, ...props }) => {
        ...
        function handleSubmit() {
            if (loading == false && name.valid) {
                actions
                    .createRecord(name.value, template?.id)
                    .then((data) => {
                        props.onClose(data, "created");
                        navigator.openRecord(data);
                    })
                    .catch(() => {
                        setLoading(false);
                    });
